...
SELECT o.navn || 'kredsen'            AS praesentation,
       ...
       INTO basic.opstillingskreds_mv
       ...
       GENERATED ALWAYS AS
       (
        setweight(to_tsvector('basic.septima_fts_config', split_part(opstillingskredsnavn, ' ', 1)),
            'A') ||
        setweight(to_tsvector('basic.septima_fts_config', split_part(opstillingskredsnavn, ' ', 2)),
            'B') ||
        setweight(to_tsvector('basic.septima_fts_config', split_part(opstillingskredsnavn, ' ', 3)),
            'C') ||
        setweight(to_tsvector('basic.septima_fts_config',
                basic.split_and_endsubstring(opstillingskredsnavn, 4)), 'D')
       ) STORED;
       ...
       GENERATED ALWAYS AS
       (
        setweight(to_tsvector('simple', fonetik.fnfonetik(split_part(opstillingskredsnavn, ' ', 1), 2)),
            'A') ||
        setweight(to_tsvector('simple', fonetik.fnfonetik(split_part(opstillingskredsnavn, ' ', 2), 2)),
            'B') ||
        setweight(to_tsvector('simple', fonetik.fnfonetik(split_part(opstillingskredsnavn, ' ', 3), 2)),
            'C') ||
        setweight(to_tsvector('simple',
                basic.split_and_endsubstring_fonetik(coalesce(opstillingskredsnavn), 4)),
            'D')
       ) STORED;
