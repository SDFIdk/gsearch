...
r.navn AS titel,
    ...
    ALTER TABLE basic.region DROP COLUMN IF EXISTS textsearchable_plain_col;
    ...
    ALTER TABLE basic.region DROP COLUMN IF EXISTS textsearchable_unaccent_col;
    ALTER TABLE basic.region
    ...
    ALTER TABLE basic.region DROP COLUMN IF EXISTS textsearchable_phonetic_col;
    ALTER TABLE basic.region
...
CREATE OR REPLACE FUNCTION api.region(input_tekst text,filters text,sortoptions integer,rowlimit integer)
    ...
    stmt = format(E'SELECT
            regionskode::text, navn::text, titel::text, geometri, bbox::geometry,
