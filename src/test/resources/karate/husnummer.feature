Feature: Gsearch husnummer test
    ...
    Scenario: Search street name that has numbers in it 2.Tværvej 2, 4970 Rødby
        Then param q = '2.Tværvej 2, 4970 Rødby'
...
Scenario: Search street name that has periods in it N. O. Hansens Vej and N.O.Hansens Vej
    ...
    Scenario: Search with supplerende bynavn returns the same as without supplerende bynavn
        Then param q = 'Småkærvej 48, Hejsager, 6100 Haderslev'

        When method GET
        And retry until responseStatus == 200
        And match response == '#[1]'
        And match response.[0].visningstekst == "Småkærvej 48, Hejsager, 6100 Haderslev"

        Then param q = 'Småkærvej 48, 6100 Haderslev'
